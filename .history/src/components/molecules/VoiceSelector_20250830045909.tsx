...
const VoiceSelector = ({ onVoiceSelect }: VoiceSelectorProps) => {
  ...
  const getVoiceDescription = (voice: any): string => {
    ...
    // Add voice code
    parts.push(`Código: ${generateVoiceCode(voice)}`);
  ...
  const generateVoiceCode = (voice: any): string => {
    const identifier = voice.identifier || '';
    const lang = voice.language || '';
    ...
    const voiceNumber = identifier.match(/\d+/);
    const numberCode = voiceNumber ? voiceNumber[0] : '1';
    
    // Generate unique code: LANG-GENDER-AGE-QUALITY-NUMBER
    return `${langCode}-${genderCode}${ageCode}-${qualityCode}-${numberCode}`;
  };
                  ...
                  >
                    ...
                    >
                      ...
                      <LayoutView hasMarginLeft isFlex1>
                        ...
                        >
                          Código: {generateVoiceCode(voice)}
                        </LayoutText>
                      </LayoutView>
                    </LayoutView>
