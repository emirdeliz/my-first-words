  ...
  const loadAvailableVoices = async () => {
    try {
      ...
      filteredVoices.forEach(voice => {
        const key = `${voice.language}-${voice.identifier}`;
        if (duplicateCheck.has(key)) {
          duplicates.push({ original: duplicateCheck.get(key), duplicate: voice });
        } else {
          duplicateCheck.set(key, voice);
        }
      });
      
      if (duplicates.length > 0) {
        console.log('⚠️ Duplicate voices found:', duplicates);
      } else {
