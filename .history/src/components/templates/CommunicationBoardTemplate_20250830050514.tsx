...
const CommunicationBoardTemplate = () => {
  ...
  // Monitor changes in selected voice specifically
  useEffect(() => {
    console.log('🎤 CommunicationBoardTemplate - Selected voice changed to:', audioConfig.selectedVoice);
  }, [audioConfig.selectedVoice]);

  // Monitor when screen comes into focus and force refresh
  useFocusEffect(
    React.useCallback(() => {
      console.log('🎤 CommunicationBoardTemplate - Screen focused, current audio config:', audioConfig);
      
      // Force refresh of audio config when screen comes into focus
  ...
  return (
    <LayoutView isFlex style={{ backgroundColor: colors.background }}>
      ...
      >
        ...
        >
          Voz: {audioConfig.selectedVoice ? `ID: ${audioConfig.selectedVoice}` : 'Padrão'}
        </LayoutText>
        <LayoutText 
          isTextXs
          style={{ color: colors.textInverse, opacity: 0.6 }}
        >
          Última atualização: {audioConfig.lastUpdated ? new Date(audioConfig.lastUpdated).toLocaleTimeString('pt-BR') : 'N/A'}
        </LayoutText>
