...
const CommunicationBoardTemplate = () => {
  ...
  useEffect(() => {
    console.log('🌐 CommunicationBoardTemplate - Translation updated:', {
      currentLanguage,
      hasBasicNeeds: !!translation.basicNeeds,
      hasBasicNeedsLevel1: !!translation.basicNeedsLevel1,
      hasEmotions: !!translation.emotions,
      hasEmotionsLevel1: !!translation.emotionsLevel1,
      hasActivities: !!translation.activities,
      hasActivitiesLevel1: !!translation.activitiesLevel1,
      hasSocial: !!translation.social,
      hasSocialLevel1: !!translation.socialLevel1,
    });
  ...
  // Function to generate a voice index
  const getVoiceIndex = (voiceId: string): string => {
    if (!voiceId) return 'N/A';
    
    // Generate a simple index based on the voice ID
    // This creates a consistent index for the same voice
    const hash = voiceId.split('').reduce((acc, char) => {
      return char.charCodeAt(0) + ((acc << 5) - acc);
    }, 0);
    
    // Convert to positive number and get last 2 digits
    const positiveHash = Math.abs(hash);
    const index = (positiveHash % 99) + 1; // 1-99 range
    
    return index.toString().padStart(2, '0'); // Always 2 digits (01, 02, etc.)
  };
    ...
    <LayoutView isFlex style={{ backgroundColor: colors.background }}>
      ...
      >
        ...
        <LayoutView style={{ flex: 1, marginLeft: 16 }}>
          <LayoutText 
            isTextSm
            style={{ color: colors.textInverse, opacity: 0.9 }}
          >
            {translation.settings?.audio?.title || 'Voz Selecionada'}
          </LayoutText>
          {audioConfig.selectedVoice ? (
            <LayoutView style={{ marginTop: 4 }}>
              <LayoutText 
                isTextXs
                style={{ color: colors.textInverse, opacity: 0.7 }}
              >
                ID: {audioConfig.selectedVoice}
              </LayoutText>
                              <LayoutText 
                  isTextXs
                  style={{ color: colors.textInverse, opacity: 0.7, marginTop: 2 }}
                >
                  Índice: #{getVoiceIndex(audioConfig.selectedVoice)}
                </LayoutText>
            </LayoutView>
          ) : (
            <LayoutText 
              isTextXs
              style={{ color: colors.textInverse, opacity: 0.7, marginTop: 4 }}
            >
              {translation.settings?.modal?.cancel || 'Padrão'}
            </LayoutText>
          )}
        </LayoutView>
        <LayoutView isFlexRow isItemsCenter>
          <TouchableOpacity 
            onPress={async () => {
              console.log('🔄 Manual refresh of audio config');
              await loadAudioConfig();
            }}
