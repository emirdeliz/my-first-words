  ...
  return (
    <LayoutView isFlex style={{ backgroundColor: colors.background }}>
      ...
      >
        ...
        >
          {translation.settings?.audio?.title || 'Voz'}: {audioConfig.selectedVoice ? `ID: ${audioConfig.selectedVoice}` : (translation.settings?.modal?.cancel || 'Padrão')}
        </LayoutText>
        <LayoutText 
          isTextXs
          style={{ color: colors.textInverse, opacity: 0.6 }}
        >
          {translation.settings?.about?.version || 'Última atualização'}: {audioConfig.lastUpdated ? new Date(audioConfig.lastUpdated).toLocaleTimeString('pt-BR') : 'N/A'}
        </LayoutText>
