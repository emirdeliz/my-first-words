...
const CommunicationBoardTemplate = () => {
  ...
  // Function to generate a friendly voice code
  const getVoiceCode = (voiceId: string): string => {
    if (!voiceId) return 'N/A';
    
    // Extract language and create a short code
    const parts = voiceId.split('-');
    if (parts.length >= 2) {
      const lang = parts[0].toUpperCase();
      const identifier = parts[parts.length - 1];
      return `${lang}-${identifier.slice(0, 3)}`;
    }
    
    // Fallback: use last 6 characters
    return voiceId.slice(-6);
  };
